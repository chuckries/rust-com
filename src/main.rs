...
mod win32 {
	...
	pub fn show_run_file_dialog() {
		...
		*/
		let mut obj: *mut IShellDispatch = null_mut();
		match unsafe { CoCreateInstance(&CLSID_ShApp, null_mut(), CLSCTX_INPROC_SERVER, &IID_IShellDispatch, transmute(&mut obj)) } {
			S_OK => (),
		...
		match unsafe { ((*(*obj).__vtable).FileRun)(transmute(obj)) } {
			S_OK => (),
			result => panic!("IShellDispatch.FileRun failed: error {:#08x}", result)
		}
	...
	const CLSCTX_INPROC_HANDLER: DWORD = 0x2;
	const CLSCTX_LOCAL_SERVER: DWORD = 0x4;
	const CLSCTX_INPROC_SERVER16: DWORD = 0x8;
	const CLSCTX_REMOTE_SERVER: DWORD = 0x10;
	const CLSCTX_INPROC_HANDLER16: DWORD = 0x20;
	const CLSCTX_RESERVED1: DWORD = 0x40;
	const CLSCTX_RESERVED2: DWORD = 0x80;
	const CLSCTX_RESERVED3: DWORD = 0x100;
	const CLSCTX_RESERVED4: DWORD = 0x200;
	const CLSCTX_NO_CODE_DOWNLOAD: DWORD = 0x400;
	const CLSCTX_RESERVED5: DWORD = 0x800;
	const CLSCTX_NO_CUSTOM_MARSHAL: DWORD = 0x1000;
	const CLSCTX_ENABLE_CODE_DOWNLOAD: DWORD = 0x2000;
	const CLSCTX_NO_FAILURE_LOG: DWORD = 0x4000;
	const CLSCTX_DISABLE_AAA: DWORD = 0x8000;
	const CLSCTX_ENABLE_AAA: DWORD = 0x10000;
	const CLSCTX_FROM_DEFAULT_CONTEXT: DWORD = 0x20000;
	const CLSCTX_ACTIVATE_32_BIT_SERVER: DWORD = 0x40000;
	const CLSCTX_ACTIVATE_64_BIT_SERVER: DWORD = 0x80000;
	const CLSCTX_ENABLE_CLOAKING: DWORD = 0x100000;
	const CLSCTX_APPCONTAINER: DWORD = 0x400000;
	const CLSCTX_ACTIVATE_AAA_AS_IU: DWORD = 0x800000;
	const CLSCTX_PS_DLL: DWORD = 0x80000000;

	const COINIT_APARTMENTTHREADED: DWORD = 0x2;
	const COINIT_MULTITHREADED: DWORD = 0x0;
	const COINIT_DISABLE_OLE1DDE: DWORD = 0x4;
	const COINIT_SPEED_OVER_MEMORY: DWORD = 0x8;

	const S_OK: HRESULT = 0x00000000;
	const S_FALSE: HRESULT = 0x00000001;

	const CLASS_E_NOAGGREGATION: HRESULT = 0x80040110;
	const E_NOINTERFACE: HRESULT = 0x80004002;
	const REGDB_E_CLASSNOTREG: HRESULT = 0x80040154;
	const REGDB_E_IIDNOTREG: HRESULT = 0x80040155;
